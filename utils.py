...
def get_traffic_info(row):
    ...
    if data:
        ...
        speed = data['routes'][0]['legs'][0]['annotation']['speed'][0]
...
def process_data(data, modality):
    data['u_coords'], data['v_coords'] = zip(
        *data['geometry'].apply(extract_coordinates))
    data = data.reset_index()
    durations = [None] * len(data)

    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(get_traffic_info, row)
    ...
    final_data['time_car'] = final_data['duration_s'] / \
